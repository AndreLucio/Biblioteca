<?php
if (!isset($_SESSION)) {
    session_start();
}

if($_SESSION['tipo'] != 4){
    header('location:index.php');
}

require_once "view/template.php";
require_once "dao/daoEditora.php";
require_once "modelo/editora.php";

template::header();
template::sidebar();
template::mainpanel();

$daoEditora = new daoEditora();

if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save") {
    $editora = new Editora();
    if(isset($_POST["id"])){
        $editora->setIdEditora($_POST["id"]);
    }
    $editora->setNomeEditora($_POST["nomeEditora"]);
    $daoEditora->salvar($editora);
    unset($editora);

}

if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "upd" && $_REQUEST["id"]) {
    $id = $_REQUEST["id"];
    $editora = new Editora();
    $editora = $daoEditora->atualizar($id);
}

if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "del" && $_REQUEST["id"]) {
    $id = $_REQUEST["id"];
    $daoEditora->remover($id);
}


?>

    <div class='content' xmlns="http://www.w3.org/1999/html">
        <div class='container-fluid'>
            <div class='row'>
                <div class='col-md-12'>
                    <div class='card'>
                        <div class='header'>
                            <h4 class='title'>Editoras</h4>
                            <p class='category'>Lista de Editoras do Sistema</p>
                        </div>
                        <div class='content table-responsive'>
                            <form action="?act=save&id=" method="POST">
                                <input type="hidden" name="id" value="<?php if(isset($editora) && $editora != null) {echo $editora->getIdEditora();}?>">
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        <label for="">Nome</label>
                                        <input type="text" name="nomeEditora" class="form-control" value="<?php if(isset($editora) && $editora != null) {echo $editora->getNomeEditora();}?>">
                                    </div>
                                    <div class="col-md-6 d-flex align-items-center">
                                        <button class='btn btn-success' type="submit">Cadastrar</button>
                                    </div>
                                </div>                                
                                
                            </form>
                        </div>
                    </div>
                </div>
            </div>
            <?php
                $daoEditora->tabelaPaginada();
            ?>
        </div>
    </div>


<?php
    template::footer();
?>



